...
test_that("rforcecom.queryall", {
  expect_equal(nrow(deleted_soql_result1), NULL)
...
test_that("Simple query returns expected columns", {
  ...
  # this test sometimes returns 'User.Id, User.Name' and sometiems 'Owner.Id, Owner.Name'
  expect_named(lookupFieldTest[,1,drop=F], c("Id")) #, "Owner.Id", "Owner.Name"))
  expect_equal(nchar(as.character(lookupFieldTest[,'Id'])), 18)  # IDs should be 18 characters long
  expect_equal(nchar(as.character(lookupFieldTest[,2])), 18)  # IDs should be 18 characters long
